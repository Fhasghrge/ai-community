...
export default function Solicit() {
  return (
    <div className="w-2/3 m-auto flex flex-col space-y-10 mt-6">
      {
        projects.map(({ title, detail, imgurl }) => (
          <ProjectItem key={imgurl} title={title} detail={detail} imgurl={imgurl} />
        ))
      }
    </div>
  )
...
function ProjectItem({ title, detail, imgurl }) {
  return (
    <div className="flex border p-4 rounded-lg shadow-lg border-gray-500">
      ...
      <div className="w-full px-4">
        ...
        <div className="text-gray-400 h-20 text-ellipsis">{detail}</div>
